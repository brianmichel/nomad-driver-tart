...
type Virtualizer interface {
	...
	// SetupVM creates a new virtual machine from a URL
	SetupVM(ctx context.Context, vmName string, url string) error
	...
	SSH(ctx context.Context, vmName, user, password, command string) (string, string, error)
