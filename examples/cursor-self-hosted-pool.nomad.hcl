job "cursor-self-hosted-worker" {
  ...
  group "workers" {
    ...
    task "vm" {
      ...
      env {
        # Non-secret worker settings.
        CURSOR_REPO_URL = "https://github.com/your-org/your-repo.git"
        CURSOR_REPO_REF = "main"
      }

      # Quick demo secret injection: render CURSOR_API_KEY from the Nomad
      # client's environment into a file that the VM startup script can source.
      #
      # For a personal self-hosted worker, use your regular Cursor API key.
      # Before running this job, set CURSOR_API_KEY in the Nomad client
      # environment. For production, prefer a Nomad variable instead.
      template {
        data = <<EOH
...
EOH
      ...
      # Startup script rendered into the secrets directory, which the driver
      # mounts into the VM automatically as /Volumes/My Shared Files/secrets.
      template {
        data = <<EOF
#!/bin/bash
set -euo pipefail
...
if [[ -n "$${CURSOR_API_KEY:-}" ]]; then
  echo "cursor: api key present=yes"
else
  echo "cursor: api key present=no"
  exit 1
fi
...
EOF
        destination = "secrets/startup.sh"
        perms       = "755"
      }
      ...
      config {
        ...
        command = "/bin/bash"
        args    = ["/Volumes/My Shared Files/secrets/startup.sh"]
      }
